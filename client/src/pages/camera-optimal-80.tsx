...
export default function CameraOptimal80() {
  ...
  // Take all 80 photos in sequence for 10×8 rectangular grid
  const captureAllPhotos = () => {
    ...
    const captureSequence = () => {
      ...
      if (photoData) {
        ...
        if (photoCount < 80) {
          setTimeout(captureSequence, 125); // 8 fps for faster capture
        }
      }
    };
  ...
  // Create 10×8 rectangular grid collage from 80 photos
  const createRectangularCollage = useCallback((photos: CapturedPhoto[]) => {
    if (photos.length !== 80 || !canvasRef.current) return;
    ...
    firstImg.onload = () => {
      ...
      // Create canvas with 10×8 grid layout, maintaining viewport aspect ratio for each rectangle
      const cols = 10;
      const rows = 8;
      
      // Calculate rectangle dimensions based on actual viewport aspect ratio
      const baseSize = 120; // Smaller base size for 80 photos
      ...
      photos.forEach((photo, index) => {
        ...
        img.onload = () => {
          ...
          // Divide the original image into 10×8 grid and take the corresponding section
    ...
    >
      ...
      {!isCapturing && !finalCollage && (
        <div className="flex flex-col items-center justify-center min-h-screen p-8">
          <h1 className="text-4xl font-bold mb-8 text-center">80-Photo Maximum Detail Collage</h1>
          <p className="text-xl mb-8 text-center max-w-2xl">
            Ultra-high resolution tessellation capturing 80 photos in a 10×8 grid at 8fps. Each micro-rectangle contributes precise detail for the most comprehensive composite image possible.
          </p>
      ...
      {showViewfinder && (
        <div className="absolute inset-0 z-10">
          {/* Live Micro-Rectangle Grid Overlay */}
          <div className="absolute inset-0 pointer-events-none">
            <svg className="w-full h-full" viewBox="0 0 10 8" preserveAspectRatio="none">
              {Array.from({ length: 80 }, (_, i) => {
                const col = i % 10;
                const row = Math.floor(i / 10);
                ...
                return (
                  <rect
                    ...
                    width={0.98}
                    height={0.98}
                    fill={captured ? 'rgba(168, 85, 247, 0.3)' : current ? 'rgba(255, 255, 0, 0.5)' : 'none'}
                    stroke={captured ? 'rgba(168, 85, 247, 0.8)' : current ? 'rgba(255, 255, 0, 1)' : 'rgba(255, 255, 255, 0.3)'}
                    strokeWidth="0.02"
            ...
            <div className="text-center">
              {isCapturingSequence && (
                <div className="text-white text-lg mb-4 font-semibold">
                  Taking photo {capturedPhotos.length + 1} of 80...
                </div>
              )}
              <button
      ...
      {finalCollage && (
        <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center p-8">
          <h1 className="text-4xl font-bold mb-8 text-center">80-Photo Maximum Detail Collage Complete!</h1>
          
          <div className="mb-8 max-w-4xl">
            <img 
              src={finalCollage} 
              alt="80-photo maximum detail collage"
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-center">
            <div className="flex gap-4">
              <a
                href={finalCollage}
                download="maximum-detail-collage-80.jpg"
